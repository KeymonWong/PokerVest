...
@interface CardBackScrollView() <UIScrollViewDelegate>
...
@implementation CardBackScrollView
...
- (void)reloadData
{
    ...
    // begin+1, begin+2是两张空白的卡牌, begin是初始窗口左边视野外的一张牌
    // end 是初始窗口右边视野外的一张牌
    //int begin = -5;
    int begin = -6;
    //int end = 1;
...
{
    if (!self.pagingEnabled) {
        [self.scrollView setContentOffset:[self contentOffsetForIndex:self.currentIndex] animated:NO];
    }
}
