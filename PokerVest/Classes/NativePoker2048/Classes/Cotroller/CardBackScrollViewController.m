...
- (void)updateView:(UIView *)view withDistanceToCenter:(CGFloat)distance scrollDirection:(ScrollDirection)direction
{
    ...
    if (ABS(distance) < self.viewSize / 2.0f) {
        // 如果是最左边的-6，而且往右拉
        if (view.tag <= -6 && direction == ScrollDirectionRight) {
            self.displayCardBackView.imageNameStr = [NSString stringWithFormat:@"%d",-6];
            self.tag = -6;
        } else if (view.tag >= 6 && direction == ScrollDirectionLeft) {
            self.displayCardBackView.imageNameStr = [NSString stringWithFormat:@"%d",6];
            self.tag = 6;
        } else {
...
- (void)setLockAndAchievementName:(NSInteger)tag
{
    ...
    switch (self.tag) {
        case -6:
        {
            ...
            NSString *labelText = @"PokerDefault";
        ...
        case -4:
        {
            ...
            NSString *labelText = @"Win 10 Times";
            btnAttributeString = nil;
            NSMutableAttributedString *attributeString = [[NSMutableAttributedString alloc] initWithString:labelText];
            [attributeString addAttributes:@{NSForegroundColorAttributeName:[UIColor blackColor],
                                             NSFontAttributeName:[UIFont fontWithName:@"Verdana-Bold" size:fontSize],
        ...
        case -3:
        {
            ...
            NSString *labelText = @"Win 20 Times";
            btnAttributeString = nil;
            NSMutableAttributedString *attributeString = [[NSMutableAttributedString alloc] initWithString:labelText];
            [attributeString addAttributes:@{NSForegroundColorAttributeName:[UIColor blackColor],
                                             NSFontAttributeName:[UIFont fontWithName:@"Verdana-Bold" size:fontSize],
        ...
        case -2:
        {
            NSInteger scoreMoreThan40 = [defaults integerForKey:kScoreMoreThan30];
            NSString *labelText = @"Score Over 30";
            btnAttributeString = nil;
            NSMutableAttributedString *attributeString = [[NSMutableAttributedString alloc] initWithString:labelText];
            [attributeString addAttributes:@{NSForegroundColorAttributeName:[UIColor blackColor],
                                             NSFontAttributeName:[UIFont fontWithName:@"Verdana-Bold" size:fontSize],
        ...
        case -1:
        {
            NSInteger scoreMoreThan50 = [defaults integerForKey:kScoreMoreThan40];
...
- (NSInteger)accomplishedAchievementNum
{
    ...
    // -8
    NSInteger scoreMoreThan30 = [defaults integerForKey:kScoreMoreThan30];
    if (scoreMoreThan30 == 1) {
        achievedNum++;
    }
