...
static const NSInteger threshold = 50;
static const NSInteger defaultCardBack = -6;
...
- (void)setupGame
{
    ...
    // 设置背景纹理图样
    UIImage *backgroundImage = [UIImage imageNamed:@"-6"];
    ...
    cardBackImageView.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"-6q"]];
    ...
    UIImageView *scoreImg = [[UIImageView alloc] initWithFrame:CGRectMake(12,14,30,30)];
    scoreImg.image = [UIImage imageNamed:@"star_score"];
    [scoreBtn addSubview:scoreImg];
    
    UILabel *scoreLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 40, btnWidth, 30)];
...
{
    [MBProgressHUD showSuccessStatus:@"当前暂无排名"];
...
- (void)resetGame
{
    ...
    for (NSInteger i = 0; i < 2; i++) {
        ...
        [self.manager insertAtRandomLocationPokerWithColor:(int)color number:(int)number completion:^{
            ...
            if (sself.manager.library.count != 0) {
...
- (void)followUp
{
    ...
    if ([self.manager userHasWon]) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(gameFinishedWithVictory:score:)]) {
...
- (void)scoreChanged:(NSInteger)newScore
{
    ...
    NSString *scoreString = [NSString stringWithFormat:@"%ld", (long)newScore];
    ...
    [attributeString addAttributes:@{NSForegroundColorAttributeName:[UIColor orangeColor], NSFontAttributeName:[UIFont fontWithName:@"Verdana-Bold" size:fontSize], NSParagraphStyleAttributeName:paragraphStyle} range:NSMakeRange(0, [scoreString length])];
