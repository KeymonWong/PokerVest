...
//  Tlm2048_1
//
//  Created by pokerVest2048 on 15/12/22.
...
- (void)setup
{
    ...
    // 在来一盘按钮
    CGFloat returnBtnHeight = winSize.height * 0.06f;
    ...
    [playAgainBtn setImage:[UIImage imageNamed:@"home"] forState:UIControlStateNormal];
    [playAgainBtn setSize:CGSizeMake(returnBtnHeight, returnBtnHeight)];
    playAgainBtn.x = winSize.width/2.0f-returnBtnHeight/2.0f;
    playAgainBtn.y = currentTop+10;
    ...
    CGFloat gameCenterWidth = playAgainBtnWidth * 0.36f;
    CGFloat gameCenterHeight = gameCenterWidth;
    CGFloat padding = winSize.width * 0.16f;
    ...
    // 返回主界面右边的分享按钮
    CGFloat shareWidth = playAgainBtnWidth * 0.36f;
    CGFloat shareHeight = shareWidth;
    UIButton *share = [[UIButton alloc] initWithFrame:CGRectMake(playAgainBtn.frame.origin.x+returnBtnHeight+padding, currentTop+returnBtnHeight/2.0f+14-shareHeight/2.0f, shareWidth, shareHeight)];
...
//        [self.gameCenterManager submitAchievement:kAchievementWinOnce percentComplete:100.0];
    ...
    NSInteger isWin100 = [defaults integerForKey:kGameWin10];
    if (self.hasWon == YES && gameWinTimes == 100 && isWin100 == 0) {
        AchivementView *achivementView = [AchivementView achivementWithPosition:CGPointZero sideWidth:achievementViewWidth sideHeight:achievementViewHeight cardBackImageStr:@"-4" achivementDiscription:@"You Win 10 Times"];
        [self.achivementArray addObject:achivementView];
//        [self.gameCenterManager submitAchievement:kAchievementWin100 percentComplete:100.0];
        [defaults setInteger:1 forKey:kGameWin10];
        NSLog(@"Reach the achivement: win 10 games");
    }
    
    // 赢得游戏500次成就，当gameWinTimes为500时获取
    NSInteger isWin500 = [defaults integerForKey:kGameWin20];
    if (self.hasWon == YES && gameWinTimes == 500 && isWin500 == 0) {
        AchivementView *achivementView = [AchivementView achivementWithPosition:CGPointZero sideWidth:achievementViewWidth sideHeight:achievementViewHeight cardBackImageStr:@"-3" achivementDiscription:@"You Win 20 Time"];
        [self.achivementArray addObject:achivementView];
//        [self.gameCenterManager submitAchievement:kAchievementWin500 percentComplete:100.0];
        [defaults setInteger:1 forKey:kGameWin20];
        NSLog(@"Reach the achivement: win 20 games");
    }
    
    // 获取积分40分以上成就
    NSInteger isGotScoreOver40 = [defaults integerForKey:kScoreMoreThan30];
    if (self.hasWon == YES && self.score >= 40 && isGotScoreOver40 == 0) {
        AchivementView *achivementView = [AchivementView achivementWithPosition:CGPointZero sideWidth:achievementViewWidth sideHeight:achievementViewHeight cardBackImageStr:@"-2" achivementDiscription:@"Score Over 40"];
        [self.achivementArray addObject:achivementView];
//        [self.gameCenterManager submitAchievement:kAchievementScoreMoreThan40 percentComplete:100.0];
        [defaults setInteger:1 forKey:kScoreMoreThan30];
        NSLog(@"Reach the achivement: game over 40");
    }
    
    // 获得积分50分以上成就
    NSInteger isGotScoreOver50 = [defaults integerForKey:kScoreMoreThan40];
    if (self.hasWon == YES && self.score >= 50 && isGotScoreOver50 == 0) {
        AchivementView *achivementView = [AchivementView achivementWithPosition:CGPointZero sideWidth:achievementViewWidth sideHeight:achievementViewHeight cardBackImageStr:@"-1" achivementDiscription:@"Score Over 50"];
        [self.achivementArray addObject:achivementView];
