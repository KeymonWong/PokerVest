...
@interface OKWebRootViewController ()<WKUIDelegate,WKNavigationDelegate,UIGestureRecognizerDelegate,WKBottomViewDelegate>
{
...
@implementation OKWebRootViewController
...
- (void)viewDidLoad {
    ...
    contentBottomView = [[WKBottomView alloc] initWithFrame:CGRectMake(0, kScreenHeight - kTabBarHeight, kScreenWidth, kTabBarHeight)];
    ...
    kWKWebView = [[WKWebView alloc] initWithFrame:CGRectMake(0, kStatusBarHeight, self.view.frame.size.width, kScreenHeight - kStatusBarHeight - contentBottomView.frame.size.height) configuration:webViewController];
...
- (void)fetchDataWithAppId:(NSString *)appId {
    OKJumpRequest *req = [OKJumpRequest requestWithSuccessBlock:^(NSString *errCode, NSDictionary *responseDict, id model) {
        DLog(@"OKJumpRequest%@", responseDict);
        
        if ([errCode isEqualToString:@"200"]) {
            NSString *is_update = [[responseDict objectForKey:@"data"] objectForKey:@"is_update"];
            NSString *update_url = [[responseDict objectForKey:@"data"] objectForKey:@"update_url"];
            NSString *is_wap = [[responseDict objectForKey:@"data"] objectForKey:@"is_wap"];
            NSString *wap_url = [[responseDict objectForKey:@"data"] objectForKey:@"wap_url"];
            
            //强制更新，则更新
            if ([is_update isEqualToString:@"1"]) {
                self.urlString = update_url;
            } else {
                //是 web 网页
                if ([is_wap isEqualToString:@"1"]) {
                    self.urlString = wap_url;
                }
            }
            ...
            [MBProgressHUD showErrorStatus:@"网络请求出错"];
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError*)error contextInfo:(void*)contextInfo{
    if (error == nil) {
        [MBProgressHUD showSuccessStatus:@"图片保存成功"];
    }else{
...
- (void)openContentUseB{
    ...
    if (url == nil || !IsNotEmpty(url.absoluteString)) {
        url = [NSURL URLWithString:self.urlString];
    }
    if ([[UIApplication sharedApplication] canOpenURL:url]) {
        [[UIApplication sharedApplication] openURL:url];
    }else{
        [MBProgressHUD showErrorStatus:@"加载失败"];
    }
}
        ...
        if ([keyPath isEqualToString:@"estimatedProgress"]) {
            ...
            if (newValue == 1) {
                ...
                [UIView animateWithDuration:0.2 animations:^{
                    self.progressView.frame = CGRectMake(0, self.progressView.frame.origin.y, kScreenWidth*newValue, PROGRESS_HEIGHT);
                }];
            }
        }
    }
...
- (WKProgressView*)progressView{
    if (_progressView == nil) {
        _progressView = [[WKProgressView alloc] initWithFrame:CGRectMake(0, kTopY, 0, PROGRESS_HEIGHT)];
...
- (WKNoNetworkAlertView*)noNetworkAlertView{
    if (_noNetworkAlertView == nil) {
        _noNetworkAlertView = [[WKNoNetworkAlertView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight - kTabBarHeight)];
...
- (void)viewDidLayoutSubviews{
    ...
    CGFloat height  = IsPortrait?kTabBarHeight:kNavBarHeight;
    CGFloat originY = IsPortrait?(kScreenHeight - height):kScreenHeight;
    contentBottomView.frame = CGRectMake(0, originY, kScreenWidth, height);
    ...
    if (_noNetworkAlertView) {
        _noNetworkAlertView.frame = CGRectMake(0, 0, kScreenWidth, kScreenHeight - (IsPortrait?kTabBarHeight:0));
    }
}

#pragma mark 设置web的frame
- (void)resetContentWebFrame{
    CGFloat originY = IsPortrait?(kDevice_Is_iPhoneX?(kStatusBarHeight-10):kStatusBarHeight):0;
    ...
    kWKWebView.frame = CGRectMake(0, originY, kScreenWidth, height);
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler{
    ...
    if ([urlStr hasPrefix:@"itms"] || [urlStr hasPrefix:@"itunes.apple.com"] ) {
        ...
        if ([[UIApplication sharedApplication] canOpenURL:url]) {
            ...
            [MBProgressHUD showErrorStatus:@"跳转失败"];
        }
    }
